...
<head>
  ...
  <style>
    ...
    /* Layout */
    .admin-wrap { max-width:1200px; margin:22px auto; display:grid; grid-template-columns: 260px 1fr; gap:18px; padding:0 16px; }
    .admin-sidebar {
    ...
    .admin-nav a { color:var(--muted); display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:10px; text-decoration:none; font-weight:600; }
    .admin-nav a svg { opacity:0.9; }
    .admin-nav a:hover { background: rgba(255,255,255,0.02); color:#fff; transform:translateX(6px); transition:all .15s ease; }

    .sidebar-footer { margin-top:18px; }

    /* Main area */
    .admin-main { min-height:100vh; padding-bottom:44px; }
    ...
    @media (max-width: 1024px){ .stats-row{ grid-template-columns: repeat(2,1fr);} .reports-grid{ grid-template-columns: 1fr; } .admin-wrap{grid-template-columns: 220px 1fr;} .report-actions{ flex-direction:row; width:auto; } }
    @media (max-width: 640px){ .admin-wrap{ grid-template-columns: 1fr; } .admin-sidebar{ position:static; height:auto; } .report-actions{ width:100%; } .action-row{ width:100%; } }
  </style>
  ...
  <div class="admin-wrap">
    ...
    <aside class="admin-sidebar" role="navigation" aria-label="Admin sidebar">
      ...
      <nav class="admin-nav" aria-label="Admin tools">
        ...
        <a href="{{ route('admin.posts.create') }}"><svg width="16" height="16" viewBox="0 0 24 24"><path d="M11 11V3h2v8h8v2h-8v8h-2v-8H3v-2h8z"/></svg> Create Post</a>
